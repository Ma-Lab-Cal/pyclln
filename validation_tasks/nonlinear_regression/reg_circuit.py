...
REPO_ROOT = next(p for p in Path(__file__).resolve().parents if (p / "device_model").is_dir())
...
def _build_training_graph(side: int) -> Tuple[nx.Graph, Dict[str, int], List[Tuple[int, int]]]:
    """Load the network topology (edges + node map) from the task's `topology.npz` data file.
    The topology is stored as data (not constructed here), so the network is defined independently
    of this script. `side` is retained for interface compatibility and validated against the file."""
    z = np.load(Path(__file__).resolve().parent / "topology.npz", allow_pickle=True)
    if int(z["side"]) != int(side):
        raise ValueError(f"topology.npz is for side={int(z['side'])}, got side={side}")
    edge_list = [(int(u), int(v)) for (u, v) in z["edges"]]
    node_map = {str(k): int(v) for k, v in zip(z["node_map_keys"], z["node_map_vals"])}
    G = nx.Graph()
    G.add_nodes_from(int(n) for n in z["nodes"])
...
def mk_switch_netlist(
    ...
    if vth_offsets is None:
        # inline parametrized wrapper (same device as the noise path; nominal instantiation)
        import sys as _sys
        _sys.path.insert(0, str(REPO_ROOT / "common"))
        import noise_model as _NZ
        lines.append(_NZ.NMOSWRAP_PARAM.rstrip("\n"))
        for edge_idx, (t_D, t_S) in enumerate(edge_list):
            ...
            lines.append(f"XNMOS t_D t_G t_S 0 NMOSWRAP")
            lines.append(f".ends e{edge_idx}")
    else:
        # device-mismatch path: per-edge VTO via the shared parametrized NMOSWRAP (templated vto, one
        # self-contained model -> a genuinely different transistor per edge; vth_offsets[i]=0 -> nominal)
        import sys as _sys
        _sys.path.insert(0, str(REPO_ROOT / "common"))
        import noise_model as _NZ
        lines.append(_NZ.NMOSWRAP_PARAM.rstrip("\n"))
        for edge_idx, (t_D, t_S) in enumerate(edge_list):
            gate_voltage = float(weights[edge_idx])
            vth = _NZ.VTO_NOM + float(vth_offsets[edge_idx])
            lines.append(f".subckt e{edge_idx} t_D t_S")
            lines.append(f"V1 t_G 0 {gate_voltage:.16f}")
            lines.append(f"XNMOS t_D t_G t_S 0 NMOSWRAP vth={vth:.6f}")
            lines.append(f".ends e{edge_idx}")
...
def _evaluate_regression(
    ...
    for i in range(X.shape[0]):
        ...
        ng.run()
        v = float(_require_finite("evaluation output", get_voltages(ng, [out_node]))[0])
        if meas_sigma and meas_sigma > 0 and rng is not None:   # additive read-out (measurement) noise
            v = v + float(rng.standard_normal()) * float(meas_sigma)
        preds[i] = v
...
def _parse_args():
    ...
    p.add_argument("--clamp-res", type=float, default=0.01)
